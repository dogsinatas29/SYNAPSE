<!--
Copyright (C) 2026 dogsinatas29
This document and project are licensed under the GNU General Public License v3.0 (GPL-3.0).
-->
...
## 7. [v0.2.17] DTR 기반 사고 제어 엔진 (Inference Control)

### 7.1 DTR 추론 프로세스 (Backend Logic)
모든 사용자 요청은 응답 전 아래의 파이프라인을 강제 통과한다.

#### 1. Pre-Processing: 복잡도 스캔
...
#### 2. Processing: 가변 추론 (Dynamic Inference)
- **Think@n 적용:** High DTR 모드 시, 내부적으로 $N$개의 추론 경로를 생성하고 상호 검증한다.
- **Early Exit:** 중간 추론 단계에서 논리적 모순 발생 시 즉시 해당 경로를 폐기(Halt)하고 새 경로를 탐색한다.

### 7.2 SYNAPSE 데이터 인터페이스 (Visual Integration)
제미나이는 응답 생성 시 아래 규격의 메타데이터를 포함하여 SYNAPSE 엔진으로 전송한다.

#### 1. 노드 데이터 스키마 (Extended intelligence)
SYNAPSE에서 관리하는 모든 노드 객체는 내부에 반드시 `intelligence` 필드를 영구적으로 소유해야 합니다.
```json
{
  "id": "node_id",
  "intelligence": {
    "dtr": 0.92,            // 생성 시 투입된 추론 밀도 (0.0 ~ 1.0)
    "density_rho": 0.88,    // 토큰 대비 정보량
    "think_at_n": 5,        // 검증된 가설 경로 수
    "confidence": 0.98      // AI의 논리적 확신도
  },
  "visual": {
    "color": "#8A2BE2",     // High DTR 고정 컬러 (Violet)
    "glow_intensity": 1.2   // 사고 밀도에 비례하는 광도
  }
}
```

### 7.3 에디터 인터랙션 후킹 (Antigravity Hook)
- **Drag & Drop:** 사용자가 코드를 드래그하여 시냅스로 투척 시: `DTR_BOOST` (0.95) 발생.
- **Manual Override:** `RULES.md` 편집 또는 인터페이스 슬라이더 조작 시 `DTR_VALUE` 강제 동기화.

### 7.4 자가 최적화 (KISS Verification)
- **KISS Validator:** 응답 출력 직전 불필요한 추상화 여부를 확인하여, Low DTR 모드 시 최단 경로 코드를 도출한다.
