...
SYNAPSE는 프로젝트 내의 모든 마크다운(`*.md`) 파일을 자동으로 스캔하고 분류합니다:
- **History**: `prompts/` 디렉토리의 파일들은 대화 기록이나 설계 결정 로그로 인식합니다.
- **Documentation**: 그 외의 `.md` 파일들은 프로젝트 문서로 처리됩니다.
- **Integration**: 이 파일들을 분석하여 코드와 설계 문서, 그리고 프롬프트 간의 연결 고리를 시각화합니다.

### 2. 자동 파일 저장 (Automatic File Saving)
생각의 흐름을 끊지 마십시오. SYNAPSE는 강력한 프롬프트 저장 메커니즘을 제공합니다:
- **Auto-Save**: 설정 활성화 시, 제목을 묻지 않고 타임스탬프와 함께 즉시 저장하여 몰입을 방해하지 않습니다.
- **Manual Control**: 정밀한 관리가 필요할 때는 수동으로 파일명을 지정할 수 있습니다.
- **Traceability**: 모든 설계 결정은 마크다운 파일로 영구 보존되어 프로젝트의 진화 과정을 추적할 수 있습니다.
