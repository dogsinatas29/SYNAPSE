...
### v0.2.11
- **✨ 다중 언어 지능화 (Python & C/C++)**:
    - **고급 해석 엔진**: 스캐닝 정확도를 대폭 향상했습니다. Python은 심층 임포트 해석을 지원하며, C/C++은 로컬과 시스템 헤더를 구분하여 분석합니다.
    - **확장된 플로우 시각화**: Flow View에서 C/C++ 제어 구조(`if`, `while`, `for`, `switch`) 분석 지원이 추가되었습니다.
    - **노드 식별 정확도**: Python 및 C/C++에서 내부 함수가 외부 노드로 식별되는 문제를 해결했습니다.
