...
- **DTR 시각화**: AI 추론 밀도가 보라색으로 빛나며, VS Code 상태바에서 인퍼런스 압력을 직접 제어합니다.
- **버퍼 클러스터 자동화**: 수동 생성된 노드가 엄격하게 관리되는 `Buffer Cluster`로 자동 스폰되며 카메라가 포커스합니다.
- **로직 편집 마스터 스위치**: 파괴적인 생성 도구(`Node`, `Connect`)는 `Edit Logic` 스위치가 활성화될 때까지 완전히 숨겨집니다.
- **안전한 노드 삭제**: 파괴적인 파일 삭제를 막기 위해 코드를 파일 단위로 하드 삭제하는 대신, 주석 처리 보존 방식을 채택했습니다.
- **상태 변경 자동 스냅샷**: 노드 생성 삭제, 연결 등의 모든 캔버스 조작은 즉각적으로 스냅샷 히스토리에 자동 각인됩니다.
- **엣지 자동 임포트**: 엣지를 승인하면 실제 `import` 문이 코드 최상단에 시각적으로 삽입됩니다. 대기/확정 배지(`?`, `!`)의 크기를 대폭 확대하여 인지성을 높였습니다.
- **상태 초기화 프로토콜**: 4단계 완전 초기화 시스템 (Disk, Memory, Canvas, Prompt).
- **데이터 위생 보장**: JSON 버퍼 인코딩 손상 및 RangeError 등을 완벽하게 제거했습니다.
### v0.2.11
