# <img src="./resources/synapse-icon.png" width="40" height="40" /> 🧠 SYNAPSE: 비주얼 아키텍처 엔진 (v0.3.22)
...
## 🔥 최신 릴리즈: v0.3.22 - 렌더링 동기화 및 지능형 툴팁 (2026-04-20)

| 기능 | 설명 | 이점 |
|-------|---------|-----|
| **렌더링 동기화** | **2D/3D 완전 일치** | WebGL 색상(RGBA), 형상, 테두리를 2D 엔진과 1:1 동기화하여 모드 전환 시 시각적 이질감을 제거했습니다. |
| **시니어의 처방** | **지능형 툴팁 보강** | 100ms 데바운스, 아이콘(📄, ⚡) 바인딩, LOD 대응 리스트 출력을 통해 정밀한 아키텍처 진단을 지원합니다. |
| **엔진 강화** | **제로-크래시 ID 매칭** | 정밀한 Stem 기반 ID 추출을 통해 엔진 블랙아웃과 null 참조 오류를 원천 차단했습니다. |
| **시각적 가시성** | **고대비 윤곽선** | 3D 셰이더 테두리 확장 및 그림자 효과 강화를 통해 복잡한 구조에서도 선명한 가독성을 제공합니다. |
| **정체성 복구** | **강력한 노드 매핑** | Dual-Match(Exact/Stem) 로직을 통해 "Unknown" 배지를 제거하고 연결 노드의 실명을 완벽히 복구했습니다. |

---

## 🔥 v0.3.21 - 시각적 일관성 및 엣지 번들링 (2026-04-18)
...
1. **확장 프로그램 설치**: `synapse-visual-architecture-v0.3.22.vsix`를 설치합니다.
...
| 버전 | 날짜 | 설명 |
| :--- | :--- | :--- |
| **v0.3.22** | 2026-04-20 | **렌더링 동기화 및 지능형 툴팁**: 2D/3D 시각적 완전 동기화, 정체성 바인딩이 적용된 데바운스 툴팁 및 LOD 대응 리스트 도입. |
