...
- **실행 흐름 추적**: 논리적 경로를 즉각적으로 시각화합니다.
- **의존성 그래프 분석**: 복잡한 모듈 간의 관계를 매핑합니다.
- **아키텍처 드리프트 감지**: 코드와 설계 간의 불일치를 찾아냅니다.

## ⚠️ 문제점

LLM 기반 시스템은 다음과 같은 심각한 구조적 문제에 직면합니다:
- **보이지 않는 흐름**: 내부 논리 경로가 추상화 뒤에 숨겨져 있습니다.
- **의존성 폭발**: 관계가 복잡해지고 관리할 수 없게 됩니다.
- **디버깅 불가**: 전체적인 구조를 디버깅하는 것은 근본적으로 어렵습니다.

## ✅ 해결책

SYNAPSE는 실행 흐름과 구조를 상호작용 가능한 그래프로 투영하여 "코드를 읽는 것"을 "아키텍처를 보는 것"으로 전환합니다.
