...
코드 분석을 통해 의존성을 자동으로 시각화합니다!

- **지원 언어**: Python, TypeScript, JavaScript
- **감지 대상**: `import`, `from ... import`, `require()` 구문
- **실시간 업데이트**: 파일 저장 시 자동으로 엣지 재생성
- **휘발성 데이터**: 자동 발견된 엣지는 메모리에만 존재 (파일에 저장 안 함)

### 클러스터링 📦

복잡한 노드들을 그룹화하여 관리하세요!

1. **다중 선택**: Shift/Ctrl + 클릭으로 여러 노드 선택
2. **우클릭 메뉴**: "Group" 선택하여 클러스터 생성
3. **Smart Push**: 클러스터 영역 내 겹치는 노드 자동 재배치
4. **접기/펴기**: 클러스터를 최소화하여 화면 정리

### 스냅샷 & 롤백 💾

중요한 설계 지점을 저장하고 복원하세요!

- **수동 스냅샷**: 우클릭 → "Take Snapshot"
- **자동 스냅샷**: 주요 변경 시 자동 저장
- **롤백**: History 패널에서 이전 상태로 복원
