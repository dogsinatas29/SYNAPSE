...
import { runBundle } from './run_b5_bundle';

type SpeciesCounts = Map<string, number>;
...
interface TopCommunitySpeciesRow {
    communityId: string;
    species?: string;
    speciesBase?: string;
    bridgeScore?: number;
...
interface SpeciesConfidenceRow {
    species: string;
    confidence: number;
}
...
interface BoundaryConfidenceHistogram {
    label: string;
    bins: HistogramBin[];
    totalCount: number;
}

interface SpeciesScoreConfidenceRow {
    species: string;
    avg: number;
    min: number;
    max: number;
...
interface InfraMeshThresholdRecommendation {
...
function runSingle(graphFilePath: string): ParsedRun {
...
function writeJsonReport(targetPath: string, payload: unknown): void {
...
export function runB5ValidationLayer(graphFilePath: string, runCount: number): void {
    ...
    for (let i = 0; i < runCount; i++) {
        ...
        const parsed = runSingle(graphFilePath);
        runs.push(parsed);
    ...
    const reportPath = path.join(__dirname, '../../report/b5_validation_layer.latest.json');
...
if (require.main === module) {
    ...
    const runCount = Math.max(1, parseIntSafe(process.argv[3]) || 3);
