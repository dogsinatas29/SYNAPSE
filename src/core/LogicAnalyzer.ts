...
export class LogicAnalyzer {
    ...
    public analyze(state: ProjectState, workspaceRoot?: string): AnalysisIssue[] {
        ...
        // 1. 고립된 노드 탐색 (Isolated Nodes) -> 오직 실행 로직 대상
        this.detectIsolatedNodes(logicNodes, logicEdges, issues);

        // 2. 순환 의존성 탐색 (Circular Dependencies) -> 오직 실행 로직 대상
        this.detectCircularDependencies(logicNodes, logicEdges, issues);

        // 3. 막다른 길 탐색 (Dead-ends) -> 오직 실행 로직 대상
        this.detectDeadEnds(logicNodes, logicEdges, issues);

        // 4. 병목 지점 탐색 (Bottlenecks) -> 오직 실행 로직 대상
        this.detectBottlenecks(logicNodes, logicEdges, issues);

        // 5. [v0.2.18.1] Schema 무결성 검증 (Schema Validation) -> 전체 대상
        this.detectSchemaViolations(nodes, edges, issues);

        // 6. [v0.2.18.2] Architecture Guardrail 검증 (The Iron Guard) -> 전체 대상
            ...
            for (const v of neighbors) {
                if (!visited.has(v)) {
                    ...
                    if (cycleStartIdx !== -1) {
                        ...
                        if (cycle.length > 1) { // Ignore self-loops (e.g., A -> A)
                            cycles.push(cycle);
                        }
                    }
                }
    ...
    let content = isKo
        ...
        : `# 🛡️ [VISUAL IMPACT] LOGIC_REPORT.md - Architecture Logic Report\n\n`;

    const criticalIssues = issues.filter(i => i.type === 'circular' || i.type === 'schema-violation' || i.type === 'architecture-violation');

    content += isKo ? `생성 일시: ${new Date().toLocaleString()}\n` : `Generated At: ${new Date().toLocaleString()}\n`;
    content += `Test Status: ${criticalIssues.length === 0 ? '✅ Pass' : '❌ Fail (Visual Indicator: Red-out)'}\n\n`;

    if (criticalIssues.length === 0 && issues.length === 0) {
        content += isKo
            ? `✅ 분석 결과, 발견된 아키텍처 결함이 없으며 시스템 형상이 매우 견고합니다.\n`
            : `✅ Analysis complete. No architectural defects found. System formation is highly robust.\n`;
    } else {
        if (criticalIssues.length === 0) {
            content += isKo
                ? `✅ 치명적인 결함은 없습니다. (하단의 최적화 힌트를 참고하세요)\n`
                : `✅ No critical defects. (See optimization hints below)\n`;
        }

        const necrosis = issues.filter(i => i.type === 'circular' || i.type === 'schema-violation' || i.type === 'architecture-violation');
        ...
        if (necrosis.length > 0) {
            ...
            content += isKo ? `- 발견된 괴사 노드가 없습니다.\n` : `- No necrosis nodes detected.\n`;
        }
        ...
        if (fractures.length > 0) {
            content += isKo
                ...
                : `- Multiple edges are fractured due to structural defects. (Visual breakage applied)\n`;
        } else {
            content += isKo ? `- 논리적 흐름이 안전하게 연결되어 있습니다.\n` : `- Logical flow is safely connected.\n`;
        }

        content += `\n## 🌡️ [Pressure Log] Inference Pressure\n`;
        const totalAnalyzedNodes = nodes.length || 1;
        const pressure = Math.min(100, Math.round((criticalIssues.length / totalAnalyzedNodes) * 100));
        
        let pressureIcon = '🟢 (Stable)';
        if (pressure >= 60) pressureIcon = '🔥 (Critical Red-out)';
        else if (pressure >= 30) pressureIcon = '🟠 (Orange Warning)';
        else if (pressure >= 10) pressureIcon = '🟡 (Yellow Caution)';

        content += `- **${isKo ? '현재 시스템 압력' : 'Current System Pressure'}**: \`${pressure}%\` ${pressureIcon}\n`;
        content += isKo
            ? `- 인퍼런스 압력이 최고조였던 지점에서 시각적 경고가 발생했습니다.\n`
            : `- Visual warnings triggered at peak inference pressure points.\n`;

        content += `\n## 🚀 [Turbo Mode Feedback]\n`;
        content += isKo
            ? `- 터보 모드(고밀도 추론) 분석을 통해 잠재적인 논리 병목 지점을 확인했습니다.\n`
            : `- Turbo Mode (high-density inference) analysis identified potential logic bottlenecks.\n`;

        // [v0.3.21] Architectural Hints section
        const hints = issues.filter(i => i.message.includes('[Hint]') || i.type === 'warning' || i.type === 'bottleneck' || i.type === 'dead-end');
        if (hints.length > 0) {
