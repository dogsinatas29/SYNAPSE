...
export class CanvasPanel {
    ...
    private async handleTestLogic() {
        ...
        try {
            ...
            const reportUri = vscode.Uri.joinPath(workspaceFolder.uri, 'synapse_report', 'LOGIC_REPORT.md');
            try {
    ...
    private async handleVirtualDebug() {
        ...
        try {
            ...
            Logger.info(`[SYNAPSE] Architecture Surgery Impact sent: ${evidence.findings.length} findings.`);
            ...
            if (analyzedNodeCount === 0) {
                ...
                vscode.window.showWarningMessage(`[SYNAPSE] Architecture Surgery found ${errorCount} Errors, ${warningCount} Warnings, and ${fractureCount} Fractured Edges.`);
            } else {
                vscode.window.showInformationMessage(`[SYNAPSE] Architecture Surgery complete: System is stable.`);
            }

            // [v0.3.34.17] Auto-open the Surgery Report generated by the laboratory CLI
            if (surgeryReportUri) {
                try {
                    await vscode.commands.executeCommand('markdown.showPreview', surgeryReportUri);
                } catch (e) {
                    Logger.warn(`[SYNAPSE] Failed to open Surgery Report Preview, falling back to editor: ${e}`);
                    await vscode.commands.executeCommand('vscode.open', surgeryReportUri, vscode.ViewColumn.Beside);
                }
            } else {
                Logger.warn(`[SYNAPSE] No Surgery Report URI returned.`);
            }
    ...
    private async handleSaveState(newState: any) {
        console.trace("[HANDLE_SAVE_STATE]");
        ...
        try {
            ...
            const clustersToSync = newState.clusters.map((uiCluster: any) => {
                ...
                return {
                    ...
                    parent_id: uiCluster.parent_id,
                    children: uiCluster.children,
    ...
    public async sendProjectState(forceReset: boolean = false, isAuthoritative: boolean = false) {
        console.count("SEND_PROJECT_STATE_CALL");
        console.trace("[SEND_PROJECT_STATE]", { forceReset, isAuthoritative });
                    ...
                    if (discoveredState.nodes!.length > 0) {
                        ...
                        if (roamers.length > 0) {
                            ...
                            if (!this._isSyncing && this._panel) {
                                ...
                                console.warn(
                                    "[SEND_PROJECT_STATE_AUTODISCOVER]",
                                    performance.now(),
